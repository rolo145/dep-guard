 ...
 */
async function findLatestOldEnoughVersion(
  packageName: string,
 ...
 */
export async function filterUpdatesByAge(
  ...
  for (const [name, suggestedVersion] of entries) {
    ...
    const safeVersion = await findLatestOldEnoughVersion(name, suggestedVersion);
